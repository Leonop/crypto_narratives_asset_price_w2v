...
def create_dict(topic_name, topic_dict):
    ...
    expanded_words = culture_dictionary.rank_by_sim(
        expanded_words, topic_dict, model
    )

    # output the dictionary
    culture_dictionary.write_dict_to_csv(
        culture_dict=expanded_words,
        file_name=str(Path(global_options.OUTPUT_FOLDER, "dict", "expanded_dict_{}.csv".format(topic_name))),
    )
    print("Dictionary saved at {}".format(str(Path(global_options.OUTPUT_FOLDER, "dict", "expanded_dict_{}.csv".format(topic_name)))))

if __name__ == "__main__":
    # use tqdm to show progress bar
    for k, v in tqdm(dict(global_options.SEED_WORDS).items()):
        topic_dict = {k: v}
        create_dict(k, topic_dict)